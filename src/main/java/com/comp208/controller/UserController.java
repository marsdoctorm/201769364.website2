...
import com.comp208.information.GoodsCarBean;
import com.comp208.entityOfUser.ShopInformationBean;
import com.comp208.entityOfUser.UserWantBean;
import com.comp208.Configuration.FileConfig;
import com.comp208.entity.*;
import com.comp208.response.BaseResponse;
import com.comp208.implementation.*;
import com.comp208.token.TokenProccessor;
import com.comp208.tool.SaveSession;
...
//        //从session中获得用户的基本信息
        ...
        if (action == 1) {
            ...
            abc.append(StringUtils.getInstance().getRandomChar()).append(System.currentTimeMillis()).append(".jpg");
            thumbnails.append(abc);
...
//            List<ShopCar> list
    ...
    public String insertGoods() {
        ...
        for (int i = 1, k = 1, j = 189; i < 1000; i++, j++, k++) {
            ...
            shopInformation.setName("1");
            shopInformation.setModified(new Date());
            shopInformation.setLevel(level);
            shopInformation.setRemark("1");
//            double price = Math.random()*1000.00+1;
            shopInformation.setPrice(new BigDecimal(price));
            shopInformation.setSort(k);
            shopInformation.setQuantity(quantity);
            shopInformation.setImage("");
